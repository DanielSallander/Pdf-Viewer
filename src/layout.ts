...
export function createHeaderContainer(self: Visual) {
    ...
    self.vertical_sep1 = document.createElement('div');
    self.vertical_sep1.id = "vertical-sep";  
    ...
    self.vertical_sep2 = document.createElement('div');
    ...
    self.exportToFileButtoncallout.textContent = "Exporting documents is only available in the licensed version";
...
function createEventListeners(self: Visual) {
    ...
    const handleZoomPlusClick = () => {
        self.zoomLevel = Math.min(3.00, +(self.zoomLevel + zoomIncrementDecrement).toFixed(2));
        self.update(self.options);
    };
    ...
    const handleExportToFileClick = () => {
        if(self.isLicensed) {
            self.host.downloadService.exportVisualsContent(self.base64encodedString, `${self.pdfFileName}.pdf`, "base64","pdf")
            .then(() => {
...
function  setupElementClasses(self: Visual) {
    ...
    self.vertical_sep1.classList.add("vertical-sep");
    self.vertical_sep2.classList.add("vertical-sep");
...
export function createPdfContainer(self: Visual) {
	self.pdfContainer = document.createElement("div");
        ...
        self.canvas = self.pdfContainer.appendChild(<HTMLCanvasElement>document.createElement('canvas'));
